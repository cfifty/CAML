...
def get_encoder(
    size, image_dim, num_classes, *, device=torch.device("cuda:0"), **kwargs: Any
):
    if size == "ultra_tiny":
        return ultra_tiny_32(image_dim, num_classes, device=device, **kwargs)
    elif size == "tiny":
...
def ultra_tiny_32(
    image_dim, num_classes, *, device=torch.device("cuda:0"), **kwargs: Any
):
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        if label_elmes:
            self.label_elmes = torch.nn.Parameter(
                get_elmes(256, num_classes), requires_grad=False
            )
        else:
            self.label_elmes = torch.nn.Parameter(
                torch.empty(num_classes, 256), requires_grad=True
            )
            torch.nn.init.kaiming_uniform_(self.label_elmes, a=math.sqrt(5))

        self.output_proj = torch.nn.Linear(
            in_features=hidden_dim, out_features=num_classes, bias=False
        )

        if GPICL:
            self.zeros_embedding = torch.zeros((1, 1, self.num_classes))
            self.positional_embedding = nn.Parameter(
                torch.zeros(1, 26, hidden_dim)
            )  # Hardcode for rebuttal.
            self.demonstration_proj = nn.Linear(
                768 + 5, 1024
            )  # Hardcode for clip: 768 clip embeddings + 5 one hot-label encodings.
    ...
    def forward(self, features, labels):
        ...
        s = labels.shape
        label_one_hot = F.one_hot(labels.unsqueeze(0), num_classes=self.num_classes).to(
            dtype=self.label_elmes.dtype
        )
        label_embeddings = label_one_hot @ (self.elmes_scale * self.label_elmes)
        batched_label_embeddings = torch.cat(
            [self.unk_emb, label_embeddings], dim=1
        ).repeat(b, 1, 1)
    ...
    def forward_imagenet(self, features, labels):
        ...
        label_embeddings = (
            ...
            @ (self.elmes_scale * self.label_elmes)
        ).reshape(b, s, -1)
    ...
    def forward_gpicl(self, features, labels, way, shot):
        ...
        batched_label_embeddings = torch.cat(
            [self.zeros_embedding, label_one_hot], dim=1
        ).repeat(b, 1, 1)
        demonstrations = torch.cat([features, batched_label_embeddings], dim=-1)

        # GPICL relies on positional embeddings to classify the point.
        demonstrations = (
            self.demonstration_proj(demonstrations)
...
class SetEncoderBlock(EncoderBlock):
    def __init__(
        ...
        self.self_attention_set = nn.MultiheadAttention(
            hidden_dim, num_heads, dropout=attention_dropout, batch_first=True
        )
        ...
        self.I = nn.Parameter(
            torch.Tensor(1, 16, hidden_dim), requires_grad=True
        )  # Hardcode to 16.
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = self.ln_1(input)
        x_set, _ = self.self_attention_set(
            query=self.I.repeat((x.shape[0], 1, 1)), key=x, value=x, need_weights=False
        )
