...
def get_model(args, fe_metadata, device):
    ...
    if args.model == "CAML":
        if "clip" in args.fe_type and args.encoder_size == "large":
            model_path = "/home/ubuntu/flowtolearn-D2NWG/data/caml_pretrained_models/CAML_CLIP/model.pth"
...
def get_test_path(args, data_path):
    ...
    if args.eval_dataset == "meta_iNat":
        ...
        raise Exception(
            f"eval dataset in classic_test: {args.eval_dataset} is not recognized."
        )
    return test_path
